package com.osio.userservice.global.config;
...
public class EmailConfig {
    ...
    @Bean
    public JavaMailSender mailSender() {    // JavaMailSender 인터페이스를 구현한 객체를 빈으로 등록하기 위함
        JavaMailSenderImpl mailSender = new JavaMailSenderImpl();   // JavaMailSender 의 구현체를 생성하고 속성 추가
        mailSender.setHost("smtp.gmail.com");   // 이메일 전송에 사용할 SMTP 서버 호스트를 설정 (구글)
        mailSender.setPort(587);
        mailSender.setUsername(username);
        mailSender.setPassword(password);

        Properties javaMailProperties = new Properties();   // JavaMail 의 속성을 설정하기 위해 Properties 객체를 생성
        ...
        javaMailProperties.put("mail.smtp.starttls.enable", "true");    // starttls(TLS 를 시작하는 명령) 사용하여 암호화된 통신을 활성화
